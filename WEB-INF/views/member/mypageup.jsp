...
<section class="bg-light">
	...
	<h2 style="text-align: center;margin-top: 20px;">회원정보수정</h2>
        <div class="container py-4">
            <div class="row align-items-center justify-content-between">
              
            </div>
     
            <form action="mypageup.do" method="post">
                ...
                <div style="display: flex; gap: 10px; align-items: baseline;">
					...
					<input type="text" class="box" id="sample6_postcode"
									...
									style="flex-basis: 70%" value="${member.memberPostalcode}"/> 
					<input type="button" id="search"
									onclick="sample6_execDaumPostcode()" value="우편번호 찾기" />
							</div>

					<input type="text" class="box" id="sample6_address"
								placeholder="주소" name="memberAdd1" value="${member.memberAdd1}"/>
					<input type="text" id="sample6_detailAddress" placeholder="상세주소"
								style="flex-basis: 70%" name="memberAdd2" value="${member.memberAdd2}"/>
					  <br>
								
                <div style="text-align: center;">
   				<div class="gender-container">   				
				<input type="radio" id="male" name="memberGender" value="남성" ${member.memberGender == '남성' ? 'checked' : ''}/>
						<label for="male">남성</label>
				<input type="radio" id="female" name="memberGender" value="여성"  ${member.memberGender == '여성' ? 'checked' : ''}/>
